onRecordUpdate((e) => {
  ...
  const origInfoExtra = e.record.original().getString('informacao_extra')
  ...
  if (infoExtra !== origInfoExtra) {
    if (infoExtra) {
      ...
      e.record.set('informacao_extra_en', '')
    }
  }

  const descExtra = e.record.getString('descricao_extra')
  const origDescExtra = e.record.original().getString('descricao_extra')

  if (descExtra !== origDescExtra) {
    if (descExtra) {
      try {
      ...
      e.record.set('descricao_extra_en', '')
    }
  }
