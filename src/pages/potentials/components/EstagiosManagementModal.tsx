...
export function EstagiosManagementModal({
  ...
  const [estagioToDelete, setEstagioToDelete] = useState<string | null>(null)
  ...
  const confirmDelete = async () => {
    if (!estagioToDelete) return
    try {
      await pb.collection('estagios_potencial').delete(estagioToDelete)
      ...
      setEstagioToDelete(null)
    }
  }

  const handleDelete = (id: string) => {
    setEstagioToDelete(id)
  }
  ...
  return (
    <>
      ...
      <AlertDialog
        open={!!estagioToDelete}
        onOpenChange={(open) => !open && setEstagioToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Excluir Estágio</AlertDialogTitle>
            <AlertDialogDescription>
              Deseja realmente excluir este estágio? Cotações neste estágio perderão o vínculo.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
